...
class LinearCG(object):
    ...
    def solve(self, matmul_closure, rhs, result=None):
        ...
        start_vecs = matmul_closure(rhs)
        Q, T = StochasticLQ(cls=type(rhs), max_iter=self.max_iter).lanczos_batch(matmul_closure, start_vecs)
        sols = []
        for i in range(len(Q)):
            Qi = Q[0]
            Ti = T[0]
            
            sols.append(Qi.matmul(torch.gesv(Qi.t().matmul(rhs), Ti)[0]).squeeze())
        if len(sols) > 1:
